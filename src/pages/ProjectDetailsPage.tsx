    ...
    {projectId && <TaskForm projectId={projectId} onTaskCreate={handleTaskCreate} />}
     ...
     {tasks && tasks.map(task => (
       <div key= {task._id} className="border p-2 h-55 mt-10 flex flex-col gap-2 rounded">
        ...
        <button className="bg-blue-500 hover:bg-blue-400 text-white font-bold border-b-4 border-blue-700 hover:border-blue-500 rounded gap" onClick={()=> setShowEditForm(task._id)}>Edit</button>
        {showEditForm === task._id && (
          <EditTaskForm projectId={projectId} task={task} onUpdate={handleTaskUpdate}/>
          )}
        <button className="bg-red-500 hover:bg-red-600 text-white font-bold border-b-4 border-red-700 hover:border-red-500 rounded" onClick={()=> handleTaskDelete(task._id)}>Delete</button>
          </div>
